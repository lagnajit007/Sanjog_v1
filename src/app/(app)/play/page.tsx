...
const BadgeCard = ({ badge }: { badge: typeof badges[0] }) => (
  <Dialog>
    <DialogTrigger asChild>
      <Card
        className={`flex cursor-pointer flex-col items-center justify-center p-4 text-center transition-all hover:shadow-lg hover:-translate-y-1 ${badge.earned ? 'bg-card border-primary/50' : 'bg-secondary/50 border-dashed opacity-60'
          }`}
      >
...
const ChallengeCard = ({ challenge, onPlay }: { challenge: Challenge, onPlay: (challenge: Challenge) => void }) => {
  return (
...
const ActiveChallengeModal = ({ challenge, isOpen, onClose }: { challenge: Challenge | null, isOpen: boolean, onClose: () => void }) => {
